...
import type { JobDefinition, SkipReason } from '../core/types.js';
import type {
  BaselineSubmission,
  BaselineAgentMetrics,
  BaselineSkipMetric,
  BaselineSkipReason,
  BaselineResultMetric,
  BaselineDurationMetric,
  BaselineModelMetric,
  BaselineAdherenceMetric,
} from '../core/types.js';
import type { SkipLedger } from '../scheduler/SkipLedger.js';
import type { JobRunHistory, JobRun } from '../scheduler/JobRunHistory.js';
 ...
 */
function mapSkipReason(reason: SkipReason): BaselineSkipReason | null {
  switch (reason) {
...
export class TelemetryCollector {
  ...
  private collectAgentMetrics(): BaselineAgentMetrics {
    ...
    const metrics: BaselineAgentMetrics = {
      version: this.deps.version,
  ...
  private collectJobMetrics(sinceHours: number, windowStart: Date, windowEnd: Date) {
  ...
  private collectSkipMetrics(sinceHours: number): BaselineSkipMetric[] {
    const rawSkips = this.deps.skipLedger.getSkips({ sinceHours });
    const counts = new Map<string, number>();

    for (const skip of rawSkips) {
      if (!SLUG_RE.test(skip.slug)) continue;
    ...
    const metrics: BaselineSkipMetric[] = [];
    for (const [key, count] of counts) {
      const [slug, reason] = key.split(':');
      metrics.push({ slug, reason: reason as BaselineSkipReason, count: cap(count) });
    }
    return metrics;
  }

  private getRunsInWindow(windowStart: Date, windowEnd: Date): JobRun[] {
  ...
  private collectResultMetrics(runs: JobRun[]): BaselineResultMetric[] {
    const slugMap = new Map<string, { success: number; error: number; timeout: number }>();

    for (const run of runs) {
      if (!SLUG_RE.test(run.slug)) continue;
      const entry = slugMap.get(run.slug) ?? { success: 0, error: 0, timeout: 0 };

      if (run.result === 'success') entry.success++;
      else if (run.result === 'failure' || run.result === 'spawn-error') entry.error++;
      else if (run.result === 'timeout') entry.timeout++;
  ...
  private collectDurationMetrics(runs: JobRun[]): BaselineDurationMetric[] {
    const slugMap = new Map<string, { totalMs: number; count: number }>();

    for (const run of runs) {
      if (!SLUG_RE.test(run.slug) || !run.durationSeconds) continue;
  ...
  private collectModelMetrics(runs: JobRun[]): BaselineModelMetric[] {
  ...
  private collectAdherenceMetrics(windowStart: Date, windowEnd: Date): BaselineAdherenceMetric[] {
   ...
   */
  private estimateExpectedRuns(job: JobDefinition, windowMs: number): number {
   ...
   */
  private resolveFeatureFlag(config: Record<string, unknown>, flag: string): boolean {
    // Check monitoring subsection
    const monitoring = config.monitoring as Record<string, unknown> | undefined;
    if (monitoring) {
      const sub = monitoring[flag] as { enabled?: boolean } | boolean | undefined;
      if (typeof sub === 'boolean') return sub;
      if (sub && typeof sub === 'object' && 'enabled' in sub) return !!sub.enabled;
    }

    // Check top-level config sections
    const section = config[flag] as { enabled?: boolean } | boolean | undefined;
    if (typeof section === 'boolean') return section;
    if (section && typeof section === 'object' && 'enabled' in section) return !!section.enabled;
...
function bucketSessions(count: number): '0' | '1-5' | '6-20' | '20+' {
  if (count === 0) return '0';
