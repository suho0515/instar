...
export class TelemetryAuth {
    ...
    provision() {
        ...
        if (!fs.existsSync(this.telemetryDir)) {
            ...
            try {
                fs.chmodSync(this.telemetryDir, 0o700);
            }
            catch { /* best-effort */ }
        }
        let created = false;
     ...
     */
    sign(installationId, timestamp, payloadBytes) {
    ...
    deprovision() {
        try {
            fs.unlinkSync(this.installIdPath);
        }
        catch { /* may not exist */ }
        try {
     ...
     */
    getInstallationIdPrefix() {
        const id = this.getInstallationId();
        return id ? id.slice(0, 8) : null;
    }
}
//# sourceMappingURL=TelemetryAuth.js.map