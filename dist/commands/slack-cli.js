 ...
 */
async function checkEventSubscriptions(wsUrl) {
 ...
 */
export async function addSlack() {
    ...
    try {
        ...
        if (!config.messaging)
            config.messaging = [];
        config.messaging = config.messaging.filter((m) => m.type !== 'slack');
...
export async function removeSlack() {
    ...
    const hasSlack = config.messaging?.some((m) => m.type === 'slack');
    if (!hasSlack) {
        console.log(pc.yellow('No Slack adapter configured.'));
        return;
    }
    const answer = await readLine(pc.yellow('This will remove all Slack config and message history. Continue? (y/N) '));
...
//# sourceMappingURL=slack-cli.js.map