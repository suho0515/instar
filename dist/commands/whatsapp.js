 ...
 */
async function isBaileysInstalled() {
...
export async function addWhatsApp(opts) {
    ...
    config.messaging = config.messaging.filter((m) => m.type !== 'whatsapp');
...
export async function channelLogin(adapter, opts) {
    if (adapter !== 'whatsapp') {
    ...
    const authMethod = opts.method ?? waConfig.config.baileys?.authMethod ?? 'qr';
...
export async function channelDoctor(adapter, opts) {
    let config;
    try {
        config = loadConfig(opts.dir);
    }
    ...
    for (const adapterType of adapters) {
        ...
        if (adapterType === 'whatsapp') {
            const waConf = adapterConfig.config;
            // Check backend
        ...
        if (adapterType === 'telegram') {
            const tgConf = adapterConfig.config;
...
export async function channelStatus(opts) {
    let config;
    try {
        config = loadConfig(opts.dir);
    }
