...
export class TreeTriage {
    ...
    threshold;
    nodeThreshold;
    constructor(intelligence, threshold) {
     ...
     */
    filterRelevantLayers(layers, scores) {
     ...
     */
    filterRelevantNodes(nodes, nodeScores) {
     ...
     */
    validateNodeIds(nodeIds, layers) {
     ...
     */
    ruleBasedFallback(query, layers, layerIds, startTime) {
        ...
        for (const layer of layers) {
            ...
            if (layerNodeScores.length > 0) {
                ...
                boostedLayerScores[layer.id] = Math.max(boostedLayerScores[layer.id] ?? 0, maxNodeScore);
            }
        }
     ...
     */
    async llmNodeTriage(query, relevantLayers) {
...
Return ONLY valid JSON mapping node IDs to scores.`;
     ...
     */
    parseNodeTriageResponse(response, nodeIds, query, layers) {
...
Return ONLY valid JSON, no explanation: {"${layers.map(l => l.id).join('": 0.0, "')}": 0.0}`;
    ...
    parseTriageResponse(response, layerIds) {
    ...
    ruleBasedLayerTriage(query, layerIds, startTime) {
        const lower = query.toLowerCase();
        const scores = {};
        for (const id of layerIds) {
...
//# sourceMappingURL=TreeTriage.js.map