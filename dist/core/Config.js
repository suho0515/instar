...
export function getInstarVersion() {
    try {
 ...
 */
export function standaloneAgentsDir() {
 ...
 */
export function resolveAgentDir(nameOrPath) {
    if (!nameOrPath) {
    ...
    try {
        ...
        if (fs.existsSync(registryPath)) {
            ...
            const entry = entries.find((e) => e.name === nameOrPath);
            if (entry?.path)
                return entry.path;
        }
    }
    catch { /* registry may not exist yet */ }
    throw new Error(`Agent "${nameOrPath}" not found. Check standalone agents at ${standaloneAgentsDir()}/ ` +
...
export function loadConfig(projectDir) {
    ...
    try {
        fileConfig = mergeConfigWithSecrets(fileConfig, stateDir);
    }
    catch {
    ...
    let contextSigningKey = fileConfig.contextSigningKey;
    ...
    if (host !== '127.0.0.1' && host !== 'localhost' && host !== '::1' && !fileConfig.authToken) {
        console.warn(`[Config] WARNING: Server binding to ${host} without authToken configured. ` +
    ...
    return {
        ...
        agentType: resolvedProjectDir.startsWith(standaloneAgentsDir())
            ? 'standalone'
            : fileConfig.agentType || 'project-bound',
        contextSigningKey,
    };
}
/**
