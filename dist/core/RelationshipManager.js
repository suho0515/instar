...
Respond with ONLY: YES or NO`;
    ...
    recordInteraction(id, interaction) {
        ...
        if (record.recentInteractions.length > this.config.maxRecentInteractions) {
            record.recentInteractions = record.recentInteractions.slice(-this.config.maxRecentInteractions);
        }
     ...
     */
    updateArcSummary(id, arcSummary) {
        const record = this.relationships.get(id);
        if (!record)
            return;
     ...
     */
    mergeRelationships(keepId, mergeId) {
        ...
        for (const channel of merge.channels) {
            if (keep.channels.length >= MAX_CHANNELS)
                break;
        ...
        for (const theme of merge.themes) {
            if (keep.themes.length >= 20)
                break;
            if (!keep.themes.includes(theme))
                keep.themes.push(theme);
        }
     ...
     */
    getStaleRelationships(daysThreshold = 14) {
        const cutoff = new Date();
        cutoff.setDate(cutoff.getDate() - daysThreshold);
        const cutoffStr = cutoff.toISOString();
        return this.getAll('recent').filter((r) => r.lastInteraction < cutoffStr && r.significance >= 3);
    }
    // ── Persistence ────────────────────────────────────────────────────
    loadAll() {
        if (!existsSync(this.config.relationshipsDir))
            return;
    ...
    calculateSignificance(record) {
        ...
        if (record.interactionCount >= 50)
            score += 4;
        else if (record.interactionCount >= 20)
            score += 3;
        else if (record.interactionCount >= 5)
            score += 2;
        else if (record.interactionCount >= 2)
            score += 1;
        ...
        if (record.themes.length >= 10)
            score += 3;
        else if (record.themes.length >= 5)
            score += 2;
        else if (record.themes.length >= 2)
            score += 1;
