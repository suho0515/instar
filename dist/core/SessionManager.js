...
export class SessionManager extends EventEmitter {
    ...
    isSessionAlive(tmuxSession) {
        ...
        try {
            const paneInfo = execFileSync(this.config.tmuxPath, ['display-message', '-t', `=${tmuxSession}:`, '-p', '#{pane_dead}||#{pane_current_command}||#{pane_start_command}'], { encoding: 'utf-8', timeout: 5000 }).trim();
    ...
    async isSessionAliveAsync(tmuxSession) {
        ...
        try {
            const { stdout } = await execFileAsync(this.config.tmuxPath, ['display-message', '-t', `=${tmuxSession}:`, '-p', '#{pane_dead}||#{pane_current_command}||#{pane_start_command}'], { timeout: 5000 });
