...
export class BaileysBackend {
    ...
    async connect() {
        try {
            ...
            let baileys = await import('baileys').catch(() => null);
            if (!baileys) {
            ...
            let version = this.config.version || undefined;
            ...
            const browser = this.config.browser || ['Mac OS', 'Chrome', '14.4.1'];
                ...
                if (qr) {
                    ...
                    if (this.config.authMethod === 'pairing-code' && this.config.pairingPhoneNumber && !this._pairingCodeRequested) {
                        ...
                        try {
                            ...
                            this.handlers.onError(new Error(`Failed to request pairing code: ${pairErr instanceof Error ? pairErr.message : pairErr}`));
                        }
                    }
                }
                if (connection === 'open') {
                ...
                for (const msg of m.messages) {
                    ...
                    if (isAudio) {
                        ...
                        this.handleAudioMessage(sock, msg, jid, senderName, timestamp, participant).catch(() => {
                            ...
                            this.handlers.onMessage(jid, msg.key.id ?? `${Date.now()}`, '[Audio]', senderName, typeof timestamp === 'number' ? timestamp : undefined, msg.key, participant, undefined);
                        });
                        continue;
                    }
                    // Extract text from various message types.
                    // For media without captions, generate a placeholder so messages aren't silently dropped.
                    const captionOrText = msg.message.conversation ??
                        msg.message.extendedTextMessage?.text ??
                        msg.message.imageMessage?.caption ??
                        msg.message.videoMessage?.caption ??
                        null;
                    const mediaPlaceholder = msg.message.imageMessage ? '[Image]' :
                    ...
                    this.handlers.onMessage(jid, msg.key.id ?? `${Date.now()}`, text, senderName, typeof timestamp === 'number' ? timestamp : undefined, msg.key, participant, mentionedJids.length > 0 ? mentionedJids : undefined);
                }
            });
        }
        catch (err) {
     ...
     */
    async handleAudioMessage(socket, msg, jid, senderName, timestamp, participant) {
        const provider = this.resolveTranscriptionProvider();
        if (!provider) {
        ...
        try {
            ...
            this.handlers.onMessage(jid, msg.key.id ?? `${Date.now()}`, `[voice] ${transcript}`, senderName, typeof timestamp === 'number' ? timestamp : undefined, msg.key, participant, undefined);
        }
        catch (transcribeErr) {
...
//# sourceMappingURL=BaileysBackend.js.map