...
export class TopicMemory {
    ...
    migrateSchema() {
        ...
        if (currentVersion < '4') {
            ...
            const columns = this.db.prepare("PRAGMA table_info(topic_summaries)").all();
            const columnNames = new Set(columns.map(c => c.name));
    ...
    insertMessage(msg) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(msg.messageId, msg.topicId, msg.text, msg.fromUser ? 1 : 0, msg.timestamp, msg.sessionName, msg.senderName ?? null, msg.senderUsername ?? null, msg.telegramUserId ?? null, msg.userId ?? null, msg.privacyScope ?? 'private');
    ...
    `);
        ...
        const tx = this.db.transaction(() => {
            for (const msg of messages) {
                const result = insert.run(msg.messageId, msg.topicId, msg.text, msg.fromUser ? 1 : 0, msg.timestamp, msg.sessionName, msg.senderName ?? null, msg.senderUsername ?? null, msg.telegramUserId ?? null, msg.userId ?? null, msg.privacyScope ?? 'private');
                if (result.changes > 0)
                    count++;
            }
        });
    ...
    getRecentMessages(topicId, limit = 20) {
      ...
      LIMIT ?
    `).all(topicId, limit).reverse().map(mapRow);
    }
    /**
     ...
     */
    getRecentMessagesForUser(topicId, userId, limit = 20) {
        if (!this.db)
            return [];
        return this.db.prepare(`
      SELECT message_id AS messageId, topic_id AS topicId, text, from_user AS fromUser,
      ...
      ORDER BY timestamp DESC, id DESC
      LIMIT ?
    `).all(topicId, userId, limit).reverse().map(mapRow);
    }
    /**
    ...
    getTopicContext(topicId, recentLimit = 20) {
        if (!this.db)
            return { summary: null, purpose: null, recentMessages: [], totalMessages: 0, topicName: null };
     ...
     */
    saveTopicSummary(topicId, summary, messageCount, lastMessageId, purpose) {
        if (!this.db)
            return;
        this.db.prepare(`
    ...
    getMessagesSinceSummary(topicId) {
      ...
      ORDER BY timestamp ASC
    `).all(topicId, lastMessageId).map(mapRow);
    }
    /**
     ...
     * Phase 2B — User-Agent Topology Spec, Gap 9
     */
    formatContextForUser(topicId, userId, recentLimit = 30) {
        if (!this.db)
            return '';
     ...
     */
    formatContextForSession(topicId, recentLimit = 50) {
        ...
        if (ctx.recentMessages.length === 0 && !ctx.summary)
            return '';
        const lines = [];
